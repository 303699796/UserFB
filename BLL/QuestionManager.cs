...
namespace UserFB.BLL
{
	...
	{
		private readonly UserFB.DAL.QuestionService dal=new UserFB.DAL.QuestionService();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public UserFB.Model.Question GetModelByCache(int questionID)
		{
			...
			return (UserFB.Model.Question)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<UserFB.Model.Question> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<UserFB.Model.Question> DataTableToList(DataTable dt)
		{
			List<UserFB.Model.Question> modelList = new List<UserFB.Model.Question>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				UserFB.Model.Question model;
